...
logging.info("Starting enhanced cat detection system with YOLOv8 and Supabase integration...")
...
DETECTION_CLASSES = os.getenv('DETECTION_CLASSES', 'cat,dog,person,bird').split(',')
...
if ENABLE_CAT_DETECTION:
    try:
        # Load YOLOv8n model (nano version for Raspberry Pi 5)
        model = YOLO('yolov8n.pt')
        logging.info("YOLOv8n model loaded successfully")
        
        # Get class names from the model
        class_names = model.names
        logging.info(f"Available classes: {len(class_names)}")
        ...
        if ENABLE_MULTI_CLASS_DETECTION:
            ...
            for cls in DETECTION_CLASSES:
                ...
                if cls in class_names.values():
                    ...
                    logging.warning(f"Class '{cls}' not found in YOLOv8 classes")
...
try:
    while True:
        ...
        if ENABLE_CAT_DETECTION and current_time >= cooldown_end_time:
            ...
            if detections:
                logging.info(f"Detected {len(detections)} objects: {[d['class'] for d in detections]}")
                timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
